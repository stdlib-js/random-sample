...
function validate( opts, options ) {
	...
	if ( hasOwnProp( options, 'mutate' ) ) {
		...
		if ( !isBoolean( opts.mutate ) ) {
			return new TypeError( format( 'invalid option. `%s` option must be a boolean primitive. Option: `%s`.', 'mutate', opts.mutate ) );
		}
	}
	if ( hasOwnProp( options, 'replace' ) ) {
		opts.replace = options.replace;
		if ( !isBoolean( opts.replace ) ) {
			return new TypeError( format( 'invalid option. `%s` option must be a boolean primitive. Option: `%s`.', 'replace', opts.replace ) );
		}
	}
	return null;
}
