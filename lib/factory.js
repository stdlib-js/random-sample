...
function factory() {
	...
	if ( arguments.length === 1 ) {
		...
		if ( !( isArrayLike( pool ) || isTypedArrayLike( pool ) ) ) {
			throw new TypeError( format( '0qd7i', 'pool', pool ) );
		}
		err = validate( conf, config );
	}
	if ( err ) {
	...
	*/
	function sample1( x, options ) {
		...
		if ( !( isArrayLike( x ) || isTypedArrayLike( x ) ) ) {
			throw new TypeError( format( '0qd38', x ) );
		}
		if ( isString( x ) ) {
			x = x.split( '' );
		}
		...
		if (
			...
			throw new RangeError( format( '0qdEG', size ) );
		}
	...
	*/
	function sample2( options ) {
		...
		if (
			...
			throw new RangeError( format( '0qdEH', size ) );
		}
		if ( replace ) {
